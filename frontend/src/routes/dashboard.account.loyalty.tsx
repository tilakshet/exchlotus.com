  ...
  return (
    <div className="flex justify-center gap-1">
      {Array.from({ length: 4 }).map((_, i) => (
        <Star key={i} className="size-5" aria-hidden="true" fill={i < count ? "var(--acc-star)" : "none"} stroke={i < count ? "var(--acc-star)" : "var(--acc-star-empty)"} />
      ))}
    </div>
  )
}
...
function TierRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between border-t border-[color:var(--acc-border-soft)] py-2.5 text-sm first:border-t-0">
...
function LevelJourney({ tiers, currentName }: { tiers: LoyaltyTier[]; currentName: string }) {
  ...
  return (
    <div className="flex items-center gap-1 overflow-x-auto rounded-[var(--acc-radius-lg)] border border-[color:var(--acc-border)] bg-[color:var(--acc-surface)] p-4">
      {tiers.map((tier, index) => {
        const isDone = index < currentIndex
        const isCurrent = index === currentIndex
        return (
          <div key={tier.name} className="flex shrink-0 items-center gap-1">
            <div className="flex flex-col items-center gap-1.5 px-2.5">
              ...
              >
                {isDone ? <Check className="size-4.5" aria-hidden="true" strokeWidth={2.3} /> : index + 1}
              </span>
              <span
            ...
            {index < tiers.length - 1 && <span aria-hidden="true" className="h-px w-9 shrink-0" style={{ background: isDone ? "var(--acc-success)" : "var(--acc-border)" }} />}
          </div>
        )
      })}
    </div>
...
function CurrentTierCard({ tier }: { tier: LoyaltyTier }) {
  ...
  return (
    <div className="w-full overflow-hidden rounded-[var(--acc-radius-lg)] border-2 bg-[color:var(--acc-surface)]" style={{ borderColor: "var(--acc-accent)" }}>
      <div className="px-4 py-5 text-center" style={{ background: "var(--acc-accent)", color: "var(--acc-accent-fg)" }}>
        <span aria-hidden="true" className="mx-auto flex size-14 items-center justify-center rounded-full" style={{ background: "rgb(255 255 255 / 25%)" }}>
          <Star className="size-7" fill="currentColor" aria-hidden="true" />
        </span>
        <TierStars count={tier.stars} />
        <p className="mt-2 text-xs font-bold uppercase tracking-wide opacity-90">Current Tier</p>
      </div>
      <div className="p-4">
        <p className="mb-1.5 flex items-center justify-between text-base font-bold text-[color:var(--acc-text-primary)]">{tier.name}</p>
        ...
        {nextTier && (
          <>
            <p className="mt-3 text-center text-xs text-[color:var(--acc-text-secondary)]">
              {formatPoints(target - CURRENT_LOYALTY_POINTS)} Points more needed to reach {nextTier.name}!
            </p>
            <div className="mt-2 h-2.5 overflow-hidden rounded-full" style={{ background: "var(--acc-border-soft)" }}>
        ...
        <Link
          ...
          className="mt-4 block rounded-[var(--acc-radius-md)] py-3 text-center text-sm font-bold outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--acc-accent)]"
  ...
  return (
    <div className="w-full rounded-[var(--acc-radius-lg)] border border-[color:var(--acc-border)] bg-[color:var(--acc-surface)]">
      <div className="px-4 py-5 text-center">
        <span aria-hidden="true" className="mx-auto flex size-14 items-center justify-center rounded-full" style={{ background: "var(--acc-accent-soft)" }}>
          <Star className="size-7" style={{ color: "var(--acc-accent)" }} fill="var(--acc-accent)" aria-hidden="true" />
        </span>
        <TierStars count={tier.stars} />
      </div>
      <div className="relative p-4">
        <p className="mb-1.5 text-base font-bold text-[color:var(--acc-text-primary)]">{tier.name}</p>
        <div aria-hidden="true" className="pointer-events-none absolute inset-x-4 top-9 bottom-15 flex items-center justify-center">
          <Lock className="size-9 text-[color:var(--acc-text-secondary)] opacity-40" aria-hidden="true" />
        </div>
